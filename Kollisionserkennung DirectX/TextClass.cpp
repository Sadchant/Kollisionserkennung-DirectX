	...
	XMMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../Kollisionserkennung DirectX/data/fontdata.txt", L"../Kollisionserkennung DirectX/data/font.dds");
	if (!result)
	{
	...
	// says Goodbye in yellow at 100, 200. The UpdateSentence function can be called to change the contents, location, and color of the
	// strings at any time.
	...
	result = UpdateSentence(m_sentence1, "Hello", 100, 100, 0.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Goodbye", 100, 200, 1.0f, 1.0f, 0.5f, deviceContext);
	if (!result)
	{
		return false;
	}
...
// UpdateSentence changes the contents of the vertex buffer for the input sentence.It uses the Map and Unmap functions along with memcpy
// to update the contents of the vertex buffer.
...
// object to draw the sentence that was given as input to this function.Notice that we use the m_baseViewMatrix instead of the current
// view matrix.This allows us to draw text to the same location on the screen each frame regardless of where the current view may be.
...
// The SetFps function takes the fps integer value given to it and then converts it to a string.Once the fps count is in a string format it
// gets concatenated to another string so it has a prefix indicating that it is the fps speed.After that it is stored in the sentence structure
// for rendering.The SetFps function also sets the color of the fps string to green if above 60 fps, yellow if below 60 fps, and red if below 30 fps.
bool TextClass::SetMs(int ms, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char msString[16];
	...
	_itoa_s(ms, tempString, 10);

	// Setup the fps string.
	//strcpy_s(fpsString, "Fps: ");
	strcpy_s(msString, "ms/Frame: ");
	strcat_s(msString, tempString);
	...
	result = UpdateSentence(m_sentence1, msString, 20, 20, red, green, blue, deviceContext);
	if (!result)
	{
		return false;
	}
...
// The SetCpu function is similar to the SetFps function.It takes the cpu value and converts it to a string which is then stored in the sentence
