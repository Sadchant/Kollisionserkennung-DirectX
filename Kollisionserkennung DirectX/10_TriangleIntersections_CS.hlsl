// Quelle: https://github.com/benardp/contours/blob/master/freestyle/view_map/triangle_triangle_intersection.c
/*		  		
...
//int coplanar_tri_tri3d(float3 p1, float3 q1, float3 r1,
//		       float3 p2, float3 q2, float3 r2,
//		       float3 normal_1, float3 normal_2){
...
//int tri_tri_intersection_test_3d(float3 p1, float3 q1, float3 r1, 
//				 float3 p2, float3 q2, float3 r2,
//				 out int coplanar, 
//				 out float3 source, out float3 target )
...
#include "TriangleIntersection.hlsl"
...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	float3 lineStart, lineEnd;
	bool coplanar;


	if (tri_tri_intersection_test_3d(vertexBuffer[triangle1[0]], vertexBuffer[triangle1[1]], vertexBuffer[triangle1[2]],
		vertexBuffer[triangle2[0]], vertexBuffer[triangle2[1]], vertexBuffer[triangle2[2]],
		coplanar,
		lineStart, lineEnd))
	{
		float3 center;
		...
		if (coplanar) 
		{
			float3 center1 = (vertexBuffer[triangle1[0]] + vertexBuffer[triangle1[1]] + vertexBuffer[triangle1[2]]) / 3;
			float3 center2 = (vertexBuffer[triangle1[0]] + vertexBuffer[triangle1[1]] + vertexBuffer[triangle1[2]]) / 3;
