...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (startLevel >= 3) // forStartNumber ist also 3, bei einem startLevel größer als 3, da die for-Scheife auf jeden Fall viermal läuft
        forStartNumber = 3;
    ...
    for (int i = forStartNumber; i >= 0; i--) // wichtig: bei 0 gibt es einen weiter Durchlauf, deswegen ist i auch kein uint
    {
            ...
            (DTid.z % threadAliveNumber == 0)) // es müssen alle drei Dimensionen geprüft werden
        {
            ...
            if (curLevel == 0) // damit nicht auf treeSizeInLevel[-1] zugegriffen wird
                curOffset = 0;
            else // der Offset wird mit dem aktuellen Level aus treeSizeInLevel geholt (treeSizeInLevel[curLevel-1] beinhaltet die Größe des Baums bis zum aktuellen Level)
            ...
            uint curChildOffset = treeSizeInLevel[curLevel].x; // der Offset für die Berechnung der 1D-IDs der Kind-Zellen
